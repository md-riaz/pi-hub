...
const recentConnections = [
  ...
  { name: "VPS", url: "https://omp-hub.riyaz.dev", token: "vps-token-demo" },
...
const remoteTree = {
  ...
  "/home/riaz/apps": ["omp-hub", "flutter-starter", "pbx-mobile"],
  ...
  "/home/riaz/apps/omp-hub": ["lib", "android", "ios", "pubspec.yaml"],
...
const baseEvents = {
  ...
  flutter: [
    { id: "f1", type: "user", text: "Clean up the OMP Hub app UI. Make it feel like ChatGPT mobile, but optimize OMP stream data better than the TUI." },
  ...
  theme: [
    { id: "t1", type: "user", text: "Generate a Flutter theme kit from our design tokens." },
    {
      ...
      items: [
        { tool: "read_file", label: "tokens/omp-hub.dark.json", meta: "done" },
    ...
    <div className="h-full flex flex-col" style={{ background: C.bg }}>
      ...
      <div className="flex-1 px-5 pt-10">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h1 className="text-center text-3xl font-semibold tracking-tight">OMP Hub</h1>
          <p className="mx-auto mt-2 max-w-[280px] text-center text-sm leading-6" style={{ color: C.text3 }}>Connect to your hub server and control OMP sessions from your phone.</p>
        </motion.div>
      ...
      <div className="border-b px-4 pb-3 pt-2" style={{ borderColor: C.softLine }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-lg font-semibold tracking-tight">OMP Hub</h1>
        ...
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 flex items-end bg-black/60" onClick={onClose}>
            <motion.div initial={{ y: 560 }} animate={{ y: 0 }} exit={{ y: 560 }} transition={{ type: "spring", damping: 30, stiffness: 260 }} className="max-h-[88%] w-full rounded-t-[32px] p-4" style={{ background: C.panel, borderTop: `1px solid ${C.line}` }} onClick={(e) => e.stopPropagation()}>
              ...
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="mb-3 h-28 w-full resize-none rounded-2xl p-3 text-sm outline-none" style={{ background: C.card, border: `1px solid ${C.line}`, color: C.text }} placeholder="Tell OMP what to do in this directory..." />
              ...
              <button onClick={start} disabled={!path.trim() || !prompt.trim()} className="flex w-full items-center justify-center gap-2 rounded-2xl py-3.5 text-sm font-bold" style={{ background: path.trim() && prompt.trim() ? C.green : C.card, color: path.trim() && prompt.trim() ? "#06110B" : C.text3 }}><Play size={17} /> Start OMP Session</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 flex items-end bg-black/60" onClick={onClose}>
          <motion.div initial={{ y: 520 }} animate={{ y: 0 }} exit={{ y: 520 }} transition={{ type: "spring", damping: 30, stiffness: 260 }} className="max-h-[86%] w-full rounded-t-[32px] p-4" style={{ background: C.panel, borderTop: `1px solid ${C.line}` }} onClick={(e) => e.stopPropagation()}>
            <div className="mx-auto mb-4 h-1 w-10 rounded-full bg-white/20" />
            <div className="mb-4 flex items-center justify-between"><div><h3 className="text-base font-semibold">Broadcast Prompt</h3><p className="text-xs" style={{ color: C.text3 }}>Send same instruction to multiple OMP sessions</p></div><button onClick={onClose} className="grid h-9 w-9 place-items-center rounded-full" style={{ background: C.card }}><X size={18} /></button></div>
...
export default function App() {
  ...
  const startNewSession = ({ path, prompt, model }) => {
    ...
    const newSession = { id, title, dir: path, repo, branch: "main", model, state: "running", activity: "Starting OMP session", files: [repo], last: "Reading repository structure", time: "now", unread: 0, accent: C.orange };
    setSessions((prev) => [newSession, ...prev]);
    setEvents((prev) => ({ ...prev, [id]: [
      { id: `${id}-u`, type: "user", text: prompt, time: "now" },
      { id: `${id}-a`, type: "assistant", text: `Starting a new OMP session in ${path}. I’ll read the repository structure first, then plan the safest next action.` },
