...
const HUB_CLIENT_NAME = "omp-hub-extension";

const DEFAULT_CONFIG: OmpHubConfig = {
	enabled: true,
	host: "0.0.0.0",
	port: 18878,
	token: "",
...
function loadConfig(): OmpHubConfig {
	mkdirSync(hubDir(), { recursive: true });
...
function serverAutoStartAllowed(config: OmpHubConfig): boolean {
	return config.autoStartServer && !isServerManuallyStopped();
}

function serverBaseUrl(config: OmpHubConfig): string {
	const host = config.host === "0.0.0.0" ? "127.0.0.1" : config.host;
	return `http://${host}:${config.port}`;
}

function getServerScript(): string {
	return join(__dirname, "omp-hub-server.mjs");
}
...
function spawnServer(config: OmpHubConfig): ChildProcess {
	mkdirSync(hubDir(), { recursive: true });
...
async function waitForServer(config: OmpHubConfig, timeoutMs = 5000): Promise<void> {
	...
	throw new Error("OMP Hub server did not start within timeout");
}

async function ensureServer(config: OmpHubConfig): Promise<void> {
	try {
		const response = await fetch(`${serverBaseUrl(config)}/api/health`, {
			headers: { authorization: `Bearer ${config.token}` },
			signal: AbortSignal.timeout(1000),
	...
	if (!config.autoStartServer) {
		throw new Error("OMP Hub server is not running and autoStartServer=false");
	}
	if (isServerManuallyStopped()) {
		throw new Error("OMP Hub server was manually stopped. Run /hub start to enable auto-start again.");
	}
...
async function post(config: OmpHubConfig, path: string, body: unknown): Promise<void> {
...
async function getJson(config: OmpHubConfig, path: string): Promise<any> {
...
function sessionHistory(ctx: ExtensionContext, limit: number): HubItem[] {
	try {
		...
		const entries = typeof manager.getEntries === "function" ? manager.getEntries() : typeof manager.getBranch === "function" ? manager.getBranch() : [];
		return entries.map((entry: any) => messageToItem(entry)).filter((item: HubItem | null): item is HubItem => Boolean(item)).slice(-limit);
	} catch {
		return [];
	}
}
...
function slashCommandSummaries(omp: ExtensionAPI): Array<{ name: string; description?: string; argumentCompletions?: Array<{ value: string; label?: string }> }> {
	try {
		return omp.getCommands().map((command: any) => {
...
function currentSessionInfo(ctx: ExtensionContext, config: OmpHubConfig, status: string, slashCommands: ReturnType<typeof slashCommandSummaries> = [], availableModels: ReturnType<typeof availableModelSummaries> = []) {
	return {
...
function networkHint(config: OmpHubConfig): string {
...
function firewallHint(config: OmpHubConfig): string {
	const port = Number(config.port);
	if (process.platform === "win32") {
		return [
			"Windows Firewall (run PowerShell as Administrator):",
			`New-NetFirewallRule -DisplayName "OMP Hub ${port}" -Direction Inbound -Action Allow -Protocol TCP -LocalPort ${port}`,
			"",
			"Remove rule:",
			`Remove-NetFirewallRule -DisplayName "OMP Hub ${port}"`,
		].join("\n");
	}
	...
	return [
		...
		`sudo ufw allow ${port}/tcp comment 'OMP Hub'`,
...
async function hubStatusText(config: OmpHubConfig, connected: boolean): Promise<string> {
	const pid = readPid();
...
function hubInfoText(config: OmpHubConfig): string {
	return [
		"═══ OMP Hub Info ═══",
		`Config: ${configPath()}`,
		`PID file: ${pidPath()}`,
...
export default function ompHubExtension(omp: ExtensionAPI) {
	...
	function sessionSlashCommands(): ReturnType<typeof slashCommandSummaries> {
		cachedSlashCommands ??= slashCommandSummaries(omp);
	...
	function setUiStatus(text: string | undefined) {
		const ctx = liveCtx();
		if (!ctx?.hasUI) return;
		try { ctx.ui.setStatus("omp-hub", text); } catch {}
	}
	...
	if (name === "hub") {
		...
		if (sub === "start") {
			...
			await register(ctx);
			notifyFallback(`OMP Hub started.\n\n${networkHint(config)}`, "info");
		} else if (sub === "stop") {
			await disconnectSession();
			notifyFallback("Disconnected this session from OMP Hub. Server still running for other sessions.", "info");
		...
		throw new Error(`Remote execution for /${name} is not supported yet. Use the OMP TUI for this slash command.`);
	}
...
async function handleCollaborationMessage(command: any): Promise<void> {
		...
		const ctx = liveCtx();
		const api = (ctx as any)?.collaboration || (omp as any).collaboration || (ctx as any)?.messageRouter || (omp as any).messageRouter;
		...
		if (!appliedToOmpApi) {
			notifyFallback([title, text, fallbackError ? `OMP collaboration API unavailable: ${fallbackError}` : ""].filter(Boolean).join("\n"), "info");
		}
		await sendEvent({
	...
	async function register(ctx: ExtensionContext): Promise<void> {
		...
		try {
			...
			setUiStatus("Hub ✗");
			if (ctx.hasUI) {
				ctx.ui.notify(`OMP Hub unavailable: ${error instanceof Error ? error.message : String(error)}`, "warning");
			}
		}
	}
				...
				try {
					...
					if (command?.type === "user_message") {
						...
						const sendOptions = deliveryMode === "followUp" || (deliveryMode === "auto" && sameSessionBusy)
							? { deliverAs: "followUp" }
							: undefined;
						await Promise.resolve(omp.sendUserMessage(content, sendOptions));
						await sendEvent({
							type: "input",
							item: {
					...
					if (command?.type === "set_model") {
						...
						await Promise.resolve(omp.setModel(model));
						await sendCommandResult(command, true);
						continue;
					}
	...
	omp.on("tool_execution_start", (event) => {
	...
	omp.on("tool_execution_update", (event) => {
		void sendEvent({
			type: "tool_update",
	...
	omp.on("tool_execution_end", (event) => {
	...
	omp.registerCommand("hub", {
		description: "OMP Hub: /hub [info|status|firewall|start|stop|server stop]",
		...
		async handler(args, ctx) {
			...
			if (sub === "start") {
				try {
					...
					startBackgroundLoops();
					await register(ctx);
					ctx.ui.notify(`OMP Hub started.\n\n${networkHint(config)}`, "info");
				} catch (error) {
					ctx.ui.notify(`OMP Hub start failed: ${error instanceof Error ? error.message : String(error)}`, "error");
				}
				return;
			}
			if (sub === "stop") {
				if (!serverOk) {
					ctx.ui.notify("This session is not connected to OMP Hub.", "warning");
					return;
				}
				await disconnectSession();
				ctx.ui.notify("Disconnected this session from OMP Hub. Server still running for other sessions.\nUse /hub start to reconnect, or /hub server stop to kill the server.", "info");
				return;
			}
			if (sub === "server stop") {
				markServerManuallyStopped();
				...
				if (pid && isProcessRunning(pid)) {
					...
					try {
						process.kill(pid);
						ctx.ui.notify(`OMP Hub server killed (PID ${pid}). Auto-restart disabled until /hub start.`, "info");
					} catch (error) {
						ctx.ui.notify(`Failed to kill server (PID ${pid}): ${error instanceof Error ? error.message : String(error)}`, "error");
					}
				} else {
					await disconnectSession();
					ctx.ui.notify("OMP Hub server is not running. Auto-restart disabled until /hub start.", "warning");
				}
				return;
			}
			if (sub === "info" || !sub) {
