...
class BroadcastResult {
  ...
  final String model;
  BroadcastResult({required this.prompt, required this.sessionIds, required this.model});
}
...
class BroadcastSheet extends StatefulWidget {
  ...
  const BroadcastSheet({super.key, required this.sessions, required this.onSend});

  static Future<void> show(BuildContext context, {required List<HubSession> sessions, required ValueChanged<BroadcastResult> onSend}) {
...
class _BroadcastSheetState extends State<BroadcastSheet> {
  ...
  Widget build(BuildContext context) {
    final canSend = _promptController.text.trim().isNotEmpty && _selected.isNotEmpty;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Broadcast Prompt', style: TextStyle(color: HubTheme.text, fontSize: 16, fontWeight: FontWeight.w600)),
                    const SizedBox(height: 4),
                    Text('Send same instruction to multiple Pi sessions', style: HubTheme.caption),
                  ],
                ),
              ),
              GestureDetector(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: m == _selectedModel ? HubTheme.blue.withOpacity(0.1) : HubTheme.card,
                      border: Border.all(color: m == _selectedModel ? HubTheme.blue.withOpacity(0.4) : HubTheme.softLine),
                      borderRadius: BorderRadius.circular(999),
                    ),
                    child: Text(m, style: TextStyle(color: m == _selectedModel ? HubTheme.blue : HubTheme.text2, fontSize: 12, fontWeight: FontWeight.w600)),
                  ),
                ),
              )).toList(),
            ),
          ...
          Row(
            children: [
              Text('Sessions', style: HubTheme.caption.copyWith(fontWeight: FontWeight.w700, letterSpacing: 0.5)),
              const Spacer(),
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: checked ? HubTheme.blue.withOpacity(0.07) : HubTheme.card,
                      border: Border.all(color: checked ? HubTheme.blue.withOpacity(0.33) : HubTheme.softLine),
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: checked ? const Icon(Icons.check_circle, size: 16, color: Color(0xFF06111F)) : null,
                        ),
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(s.displayName, style: const TextStyle(color: HubTheme.text, fontSize: 14, fontWeight: FontWeight.w600)),
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text('Send to ${_selected.length} sessions', style: TextStyle(color: canSend ? const Color(0xFF06111F) : HubTheme.text3, fontSize: 14, fontWeight: FontWeight.w700)),
                ],
              ),
            ),
          ),
