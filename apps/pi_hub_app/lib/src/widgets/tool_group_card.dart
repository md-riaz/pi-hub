...
class _ToolGroupCardState extends State<ToolGroupCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final collapsedLabel = widget.event.metadata['collapsedLabel'] ?? '${items.length} operations';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: HubTheme.card,
        border: Border.all(
          color: isRunning ? HubTheme.cyan.withOpacity(0.33) : HubTheme.softLine,
        ),
        ...
        children: [
              ...
              children: [
                Container(
                  ...
                  child: isRunning
                      ? const SizedBox(
                          width: 14,
                          height: 14,
                          child: CircularProgressIndicator(strokeWidth: 2, color: HubTheme.cyan),
                        )
                      : const Icon(Icons.check_circle_outline, size: 14, color: HubTheme.green),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(title, style: const TextStyle(color: HubTheme.text, fontSize: 12, fontWeight: FontWeight.w600)),
          ...
          if (_expanded) ...[
            ...
            ...shown.map((item) => Container(
              ...
              child: Row(
                children: [
                  Icon(_toolIcon(item['tool'] ?? ''), size: 13, color: HubTheme.blue),
  ...
  List<Map<String, String>> _parseItems() {
    ...
    if (raw is List) {
      return raw.map<Map<String, String>>((e) => Map<String, String>.from(e)).toList();
    }
    return [];
  }
  ...
  IconData _toolIcon(String tool) {
    switch (tool) {
      case 'read_file': return Icons.description_outlined;
      case 'bash': return Icons.terminal;
      case 'grep': return Icons.search;
      case 'write_file': return Icons.code;
      case 'git_diff': return Icons.account_tree;
      default: return Icons.keyboard_command_key;
    }
  }
}
