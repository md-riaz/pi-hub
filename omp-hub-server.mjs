...
const OMP_HOME = process.env.PI_CODING_AGENT_DIR || path.join(os.homedir(), ".omp", "agent");
const HUB_DIR = path.join(OMP_HOME, "omp-hub");
const CONFIG_PATH = path.join(HUB_DIR, "config.json");
const PID_PATH = path.join(HUB_DIR, "server.pid");

const SERVER_VERSION = "2.0.45";
...
function normalizeAgentCreationConfig(value = {}) {
  ...
  return {
    ompCommand: typeof source.ompCommand === "string" && source.ompCommand.trim()
      ? source.ompCommand.trim()
      : DEFAULT_CONFIG.agentCreation.ompCommand,
...
function normalizeEvent(input = {}, fallbackSessionId, fallbackType = "session.event") {
  ...
  const normalized = {
    ...
    actor: source.actor && typeof source.actor === "object" ? source.actor : { kind: sessionId ? "agent" : "server", id: sessionId || "omp-hub" },
...
function startAgentCreation(request) {
  ...
  const env = {
    ...process.env,
    OMP_HUB_AGENT_CWD: request.cwd,
    OMP_HUB_AGENT_NAME: request.name,
    OMP_HUB_AGENT_MODEL: request.model,
    OMP_HUB_INITIAL_PROMPT: request.initialPrompt,
  };
  const args = agentCreationArgs(request);
  const child = process.platform === "win32" && !config.agentCreation.testMode
    ? spawn("cmd.exe", ["/c", "start", "OMP Hub Agent", config.agentCreation.ompCommand, ...args], {
        cwd: request.cwd,
        env,
        shell: false,
        windowsHide: false,
    ...
    : spawn(config.agentCreation.ompCommand, args, {
        cwd: request.cwd,
        env,
        shell: false,
        windowsHide: false,
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/") {
    sendText(req, res, 200, [
      "OMP Hub server running.",
      `Port: ${config.port}`,
      `LAN: ${localAddresses().map(ip => `http://${ip}:${config.port}`).join(", ") || "none"}`,
      "Use Flutter OMP Hub app with token from ~/.omp/agent/omp-hub/config.json.",
    ].join("\n"));
    return;
  }
...
server.on("error", error => {
  console.error("OMP Hub server error:", error);
  process.exit(1);
});
...
server.listen(Number(config.port), config.host, () => {
  ...
  console.log(`OMP Hub server listening on ${config.host}:${config.port}`);
